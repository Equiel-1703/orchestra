...
Orchestra.defmodule ArraySum do
  ...
  def sum(input_1, input_2) do
    Orchestra.with Orchestra.cpu() do
      ...
      result = Orchestra.tensor(shape, type)
...
data = Enum.to_list(1..100) |> Enum.map(fn x -> x * 1.0 end)

a = Orchestra.tensor(data, type: {:f, 32})
b = Orchestra.tensor(data, type: {:f, 32})

result = ArraySum.sum(a, b)

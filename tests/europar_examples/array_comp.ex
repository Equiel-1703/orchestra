...
OCLPolyHok.defmodule PMap do
 ...
 def comp_func(arr1,arr2,size,f) do
    d_arr1 = OCLPolyHok.new_gnx(arr1)
    d_arr2 = OCLPolyHok.new_gnx(arr2)
    shape = OCLPolyHok.get_shape(d_arr1)
    type = OCLPolyHok.get_type(d_arr1)
    result_gpu = OCLPolyHok.new_gnx(shape,type)

    threadsPerBlock = 128;
    numberOfBlocks = div(size + threadsPerBlock - 1, threadsPerBlock)

    OCLPolyHok.spawn(&PMap.map_comp/5,
              {numberOfBlocks,1,1},
              {threadsPerBlock,1,1},
              [d_arr1,d_arr2,result_gpu,size, f])
    OCLPolyHok.get_gnx result_gpu
 end
end
...
host_resp = OCLPolyHok.gpu_for n <- a,  do: n * n

IO.inspect host_resp

host_resp = OCLPolyHok.gpu_for i <- 0..size, a, b, do:  2 * a[i] + b[i]

defmodule OrchestraFunctions do
  defguard is_atomic_type(t) when t in [:tatomic_int, :tatomic_float, :tatomic_double]

  # The functions_lookup map is used to store the name, return type, argument types and the generated OpenCL name
  # of Orchestra DSL built-in functions. If the function does not have a different name in OpenCL, you can leave
  # the opencl_name field as nil, and it will be generated by the backend using the function name.
  @functions_lookup %{
    get_global_id: %{
      return_type: :int,
      arg_types: [:int]
    },
    get_global_size: %{
      return_type: :int,
      arg_types: [:int]
    },
    get_local_id: %{
      return_type: :int,
      arg_types: [:int]
    },
    get_local_size: %{
      return_type: :int,
      arg_types: [:int]
    },
    get_group_id: %{
      return_type: :int,
      arg_types: [:int]
    },
    get_num_groups: %{
      return_type: :int,
      arg_types: [:int]
    },
    sqrt: %{
      return_type: :float,
      arg_types: [:float]
    },
    sqrtf: %{
      return_type: :float,
      arg_types: [:float],
      opencl_name: "sqrt"
    },
    max: %{
      return_type: :float,
      arg_types: [:float, :float]
    },
    fmaxf: %{
      return_type: :float,
      arg_types: [:float, :float]
    },
    min: %{
      return_type: :float,
      arg_types: [:float, :float]
    },
    fminf: %{
      return_type: :float,
      arg_types: [:float, :float]
    },
    cas_int: %{
      return_type: :int,
      arg_types: [:int, :int, :int]
    },
    cas_float: %{
      return_type: :float,
      arg_types: [:float, :float, :float]
    },
    cas_double: %{
      return_type: :double,
      arg_types: [:double, :double, :double]
    },
    sin: %{
      return_type: :float,
      arg_types: [:float]
    },
    cos: %{
      return_type: :float,
      arg_types: [:float]
    },
    tan: %{
      return_type: :float,
      arg_types: [:float]
    },
    exp: %{
      return_type: :float,
      arg_types: [:float]
    },
    log: %{
      return_type: :float,
      arg_types: [:float]
    },
    pow: %{
      return_type: :float,
      arg_types: [:float, :float]
    },
    trunc: %{
      return_type: :int,
      arg_types: [:float]
    },
    abs: %{
      return_type: :int,
      arg_types: [:int]
    },
    fabs: %{
      return_type: :float,
      arg_types: [:float]
    },
    fmod: %{
      return_type: :float,
      arg_types: [:float, :float]
    },
    clamp: %{
      return_type: :float,
      arg_types: [:float, :float, :float]
    },
    __syncthreads: %{
      return_type: :unit,
      arg_types: []
    },
    # --- Atomics ---
    atomic_add_int: %{
      return_type: :int,
      arg_types: [:tatomic_int, :int],
      opencl_name: "atomic_fetch_add"
    },
    atomic_add_float: %{
      return_type: :float,
      arg_types: [:tatomic_float, :float],
      opencl_name: "atomic_fetch_add"
    },
    atomic_add_double: %{
      return_type: :double,
      arg_types: [:tatomic_double, :double],
      opencl_name: "atomic_fetch_add"
    },
    atomic_sub_int: %{
      return_type: :int,
      arg_types: [:tatomic_int, :int],
      opencl_name: "atomic_fetch_sub"
    },
    atomic_sub_float: %{
      return_type: :float,
      arg_types: [:tatomic_float, :float],
      opencl_name: "atomic_fetch_sub"
    },
    atomic_sub_double: %{
      return_type: :double,
      arg_types: [:tatomic_double, :double],
      opencl_name: "atomic_fetch_sub"
    },
    atomic_exchange_int: %{
      return_type: :int,
      arg_types: [:tatomic_int, :int],
      opencl_name: "atomic_exchange"
    },
    atomic_exchange_float: %{
      return_type: :float,
      arg_types: [:tatomic_float, :float],
      opencl_name: "atomic_exchange"
    },
    atomic_exchange_double: %{
      return_type: :double,
      arg_types: [:tatomic_double, :double],
      opencl_name: "atomic_exchange"
    },
    atomic_min_int: %{
      return_type: :int,
      arg_types: [:tatomic_int, :int],
      opencl_name: "atomic_fetch_min"
    },
    atomic_min_float: %{
      return_type: :float,
      arg_types: [:tatomic_float, :float],
      opencl_name: "atomic_fetch_min"
    },
    atomic_min_double: %{
      return_type: :double,
      arg_types: [:tatomic_double, :double],
      opencl_name: "atomic_fetch_min"
    },
    atomic_max_int: %{
      return_type: :int,
      arg_types: [:tatomic_int, :int],
      opencl_name: "atomic_fetch_max"
    },
    atomic_max_float: %{
      return_type: :float,
      arg_types: [:tatomic_float, :float],
      opencl_name: "atomic_fetch_max"
    },
    atomic_max_double: %{
      return_type: :double,
      arg_types: [:tatomic_double, :double],
      opencl_name: "atomic_fetch_max"
    },
    atomic_and: %{
      return_type: :int,
      arg_types: [:tatomic_int, :int],
      opencl_name: "atomic_fetch_and"
    },
    atomic_or: %{
      return_type: :int,
      arg_types: [:tatomic_int, :int],
      opencl_name: "atomic_fetch_or"
    },
    atomic_xor: %{
      return_type: :int,
      arg_types: [:tatomic_int, :int],
      opencl_name: "atomic_fetch_xor"
    }
  }

  # Create a lookup function clause for each function in the functions_lookup map
  for {fun_name, info} <- @functions_lookup do
    def lookup(unquote(fun_name)) do
      unquote(Macro.escape(info))
    end

    def exists?(unquote(fun_name)) do
      true
    end
  end

  # Clauses when the function is not found in the lookup
  def lookup(_fun) do
    nil
  end

  def exists?(_fun) do
    false
  end

  # # Atomics functions map. Here, we have a list of possible type signatures where we choose the correct one based on the type of
  # # the first argument
  # @atomics_functions %{
  #   atomic_add: [
  #     {:int, [:tatomic_int, :int]},
  #     {:float, [:tatomic_float, :float]},
  #     {:double, [:tatomic_double, :double]}
  #   ],
  #   atomic_sub: [
  #     {:int, [:tatomic_int, :int]},
  #     {:float, [:tatomic_float, :float]},
  #     {:double, [:tatomic_double, :double]}
  #   ],
  #   atomic_exchange: [
  #     {:int, [:tatomic_int, :int]},
  #     {:float, [:tatomic_float, :float]},
  #     {:double, [:tatomic_double, :double]}
  #   ],
  #   atomic_min: [
  #     {:int, [:tatomic_int, :int]},
  #     {:float, [:tatomic_float, :float]},
  #     {:double, [:tatomic_double, :double]}
  #   ],
  #   atomic_max: [
  #     {:int, [:tatomic_int, :int]},
  #     {:float, [:tatomic_float, :float]},
  #     {:double, [:tatomic_double, :double]}
  #   ],
  #   atomic_and: [
  #     {:int, [:tatomic_int, :int]}
  #   ],
  #   atomic_or: [
  #     {:int, [:tatomic_int, :int]}
  #   ],
  #   atomic_xor: [
  #     {:int, [:tatomic_int, :int]}
  #   ]
  # }

  # # This is the special functions map, which contains the name and type signature of special functions
  # # that are part of OpenCL and CUDA runtime
  # @special_functions Map.new([
  #                      {:get_global_id, {:int, [:int]}},
  #                      {:get_global_size, {:int, [:int]}},
  #                      {:get_local_id, {:int, [:int]}},
  #                      {:get_local_size, {:int, [:int]}},
  #                      {:get_group_id, {:int, [:int]}},
  #                      {:get_num_groups, {:int, [:int]}},
  #                      {:sqrt, {:float, [:float]}},
  #                      {:sqrtf, {:float, [:float]}},
  #                      {:max, {:float, [:float, :float]}},
  #                      {:fmaxf, {:float, [:float, :float]}},
  #                      {:min, {:float, [:float, :float]}},
  #                      {:fminf, {:float, [:float, :float]}},
  #                      {:cas_int, {:int, [:int, :int, :int]}},
  #                      {:cas_float, {:float, [:float, :float, :float]}},
  #                      {:cas_double, {:double, [:double, :double, :double]}},
  #                      {:sin, {:float, [:float]}},
  #                      {:cos, {:float, [:float]}},
  #                      {:tan, {:float, [:float]}},
  #                      {:exp, {:float, [:float]}},
  #                      {:log, {:float, [:float]}},
  #                      {:pow, {:float, [:float, :float]}},
  #                      {:trunc, {:int, [:float]}},
  #                      {:abs, {:int, [:int]}},
  #                      {:fabs, {:float, [:float]}},
  #                      {:fmod, {:float, [:float, :float]}},
  #                      {:clamp, {:float, [:float, :float, :float]}},
  #                      {:__syncthreads, {:unit, []}}
  #                    ])
end
